...
module DxLib
  # Functions
  module Functions # rubocop:disable Metrics/ModuleLength
    header = <<~FUNCLIST
      ...
      int  __stdcall dx_GetHostIPbyNameWithStrLen( const TCHAR * HostName, size_t  HostNameLength, IPDATA *  IPDataBuf);
      int  __stdcall dx_GetHostIPbyName_IPv6( const TCHAR * HostName, IPDATA_IPv6 *  IPDataBuf);
      int  __stdcall dx_GetHostIPbyName_IPv6WithStrLen( const TCHAR * HostName, size_t  HostNameLength, IPDATA_IPv6 *  IPDataBuf);
      ...
      void  __stdcall dx_HashSha256( const void * SrcData, size_t  SrcDataSize, void *  DestBuffer);
      ...
      int __stdcall dx_GetResourceInfoWithStrLen( const TCHAR * ResourceName , size_t ResourceNameLength , const TCHAR * ResourceType , size_t ResourceTypeLength , void * * DataPointerP , int * DataSizeP ) ;
    ...
    FUNCLIST
    header.lines.compact.each do |sig|
      sig.chomp!
      md = sig.match(/(.+)__stdcall\s+dx_(.*)\((.*)\)/)
      param = md[3].split(',').map { |s| s.split('=')[0] }
      sig = "#{md[1]} #{md[2]}(#{param.join ','})"
      begin
        dx_extern sig
      rescue Fiddle::DLError => e
        puts "typealias \"#{e.message[/unknown type:(.*)$/, 1].strip}\", 'uintptr_t'"
      end
    end
  end
end
